...
for i = 1:iteration_num
    ...
    for j = 1:size(Points_X_all,1)
        if Points_X_all(j) >= (column_id-3) && ...
                ...
                Points_Y_all(j) >= (row_id-3) && ...
                Points_Y_all(j) < (row_id+2)
            X_temp = Points_X_all(j) - column_id + 2;
            Y_temp = Points_Y_all(j) - row_id + 2;
            if X_temp >= 1 && X_temp < 2 && Y_temp >= 1 && Y_temp < 2
                count_temp = count_temp + 1;
            end
            %Manually change the value here for the weight of surrounding
            %pixels. 
            count_temp = count_temp + 1;
